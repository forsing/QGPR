...
def full_qcbm(params_list, values):
    ...
    # v2: u kolu je num_positions*num_qubits = 25 (qc25), ne 35
    qc.measure_all()
...
_out_dir = Path(__file__).resolve().parent / "QGPR_qc25_7_v2_out"
_out_dir.mkdir(parents=True, exist_ok=True)
img4.savefig(str(_out_dir / "QGPR_qc25_7_4.jpg"))
...
# v2: ako import padne, uskladi qiskit-machine-learning sa instaliranim qiskit (npr. >=0.9.0 uz noviji qiskit; uz 1.4.4 drži par iz tabele u zaglavlju).
try:
    from qiskit_machine_learning.kernels import FidelityQuantumKernel
except ImportError as _e:
    raise ImportError(
        "QGPR_qc25_7_v2: pip install 'qiskit-machine-learning>=0.9.0' ili uskladi verzije sa qiskit iz zaglavlja. "
        + repr(_e)
    ) from _e
...
# v2: [0,1] pre ZZFeatureMap — sirovi indeksi često guraju GPR predikciju ka 0 → 1,2,3,4,5,6,…
_X_scaler = MinMaxScaler(feature_range=(0, 1))
X_q = _X_scaler.fit_transform(X)
_last_row = df_indexed.iloc[-1:].values
last_q = _X_scaler.transform(_last_row)

# Quantum kernel (qml 0.9+: obavezan feature_map; 7 kolona = 7 klasičnih obeležja)
_feature_map = ZZFeatureMap(feature_dimension=X_q.shape[1], reps=2, entanglement="linear")
quantum_kernel = FidelityQuantumKernel(feature_map=_feature_map)

# jednom (ista matrica za svih 7 pozicija)
K_train = quantum_kernel.evaluate(X_q, X_q)
...
# ------------------------------------------------------------
last_input = _last_row  # ostaje za štampu / debug; predikcija koristi last_q

predicted_indexed = []
for pos in range(7):
    K_pred = quantum_kernel.evaluate(last_q, X_q)
